...
class ScannerApp(ctk.CTk):
    ...
    def create_scanner_controls(self):
        ...
        # Tools Section
        self._section_header(self.scanner_frame, "✨ Image Tools")
        
        # Tool Grid
        tool_frame = ctk.CTkFrame(self.scanner_frame, fg_color="transparent")
        ...
        # Sliders with labels
        self._slider_control(self.scanner_frame, "💡 Brightness", self.update_brightness)
        self.bright_slider = self.scanner_frame.winfo_children()[-1]
        self.bright_slider.set(1.0)
        
        self._slider_control(self.scanner_frame, "🎭 Contrast", self.update_contrast)
        self.cont_slider = self.scanner_frame.winfo_children()[-1]
        self.cont_slider.set(1.0)
        
        # Advanced Actions
        self._section_header(self.scanner_frame, "🔧 Advanced")
        ...
        for text, cmd, color in actions:
            self._action_button(self.scanner_frame, text, cmd, color)
        
        self.gray_switch = ctk.CTkSwitch(self.scanner_frame, text="⚫ Black & White", 
                                        command=self.toggle_grayscale, font=("Segoe UI", 12))
        self.gray_switch.pack(anchor="w", pady=(10, 20))
        
        # Watermark / Stamp Section
        self._section_header(self.scanner_frame, "🏷️ Watermark / Stamp")
        ...
        watermark_menu = ctk.CTkOptionMenu(self.scanner_frame, variable=self.watermark_text,
        ...
        position_menu = ctk.CTkOptionMenu(self.scanner_frame, variable=self.watermark_position,
        ...
        self._slider_control(self.scanner_frame, "💧 Opacity", self.update_watermark_preview)
        self.watermark_opacity_slider = self.scanner_frame.winfo_children()[-1]
        self.watermark_opacity_slider.set(0.5)  # 50% opacity
        
        # Apply watermark button
        ctk.CTkButton(self.scanner_frame, text="🏷️ Add Watermark", command=self.apply_watermark,
                     height=40, corner_radius=10, fg_color="#DC2626",
                     hover_color="#B91C1C", font=("Segoe UI", 12, "bold")).pack(fill="x", pady=(0, 20))
        
        # Paper Size Section
        self._section_header(self.scanner_frame, "📏 Paper Size")
        ...
        size_menu = ctk.CTkOptionMenu(self.scanner_frame, variable=self.paper_size_var,
        ...
        ctk.CTkButton(self.scanner_frame, text="✂️ Resize to Selected", command=self.resize_to_paper_size,
                     height=40, corner_radius=10, fg_color="#0891B2",
                     hover_color="#0E7490", font=("Segoe UI", 12, "bold")).pack(fill="x", pady=(0, 20))
        
        # Book Tools
        self._section_header(self.scanner_frame, "📚 Book & Duplex")
        book_actions = [
        ...
        for text, cmd in book_actions:
            self._action_button(self.scanner_frame, text, cmd, "#6366F1")
        
        # Collage & Photo Grid
        self._section_header(self.scanner_frame, "🎨 Collage & Photo Grid")
        
        self.grid_layout_var = tk.StringVar(value="2x2")
        grid_layouts = ["1x2", "2x1", "2x2", "3x2", "2x3", "3x3", "4x4"]
        
        grid_menu = ctk.CTkOptionMenu(self.scanner_frame, variable=self.grid_layout_var,
        ...
        ctk.CTkButton(self.scanner_frame, text="🖼️ Create Grid from All Pages", 
        ...
        # Export
        self._section_header(self.scanner_frame, "💾 Export")
        
        self._input_field(self.scanner_frame, "Filename", self.filename_prefix)
        self._input_field(self.scanner_frame, "Location", self.output_dir)
        
        ctk.CTkButton(self.scanner_frame, text="📁 Browse...", command=self.browse_folder, height=36,
        ...
        self.save_img_btn = ctk.CTkButton(self.scanner_frame, text="🖼️ Save as Image", 
        ...
        self.save_pdf_btn = ctk.CTkButton(self.scanner_frame, text="📄 Save as PDF",
        ...
        self._section_header(self.scanner_frame, "📂 History & Files")
        ...
        for text, cmd in history_actions:
            ctk.CTkButton(self.scanner_frame, text=text, command=cmd, height=38, corner_radius=10,
                         fg_color="transparent", border_width=2, border_color=COLORS["border"],
                         text_color=COLORS["text"], hover_color=COLORS["bg_gradient_end"],
                         font=("Segoe UI", 11)).pack(fill="x", pady=3)
        
        # About
        self._section_header(self.scanner_frame, "ℹ️ About")
        ctk.CTkLabel(self.scanner_frame, text="Developed by Ilham Maulana", 
                    font=("Segoe UI", 11), text_color=COLORS["text"], anchor="w").pack(fill="x")
        ctk.CTkLabel(self.scanner_frame, text="k4ilham@gmail.com",
                    font=("Segoe UI", 10), text_color=COLORS["text_light"], anchor="w").pack(fill="x")
        
        link = ctk.CTkLabel(self.scanner_frame, text="🌐 inercorp.com", font=("Segoe UI", 11, "underline"),
        ...
        ctk.CTkButton(self.scanner_frame, text="🎧 User Guide", command=self.start_interactive_guide,
        ...
        self.thumb_self.scanner_frame = ctk.CTkScrollableFrame(right_card, fg_color="transparent")
        self.thumb_self.scanner_frame.grid(row=1, column=0, sticky="nsew", padx=15, pady=(0, 10))
    ...
    def update_thumbnails(self):
        for w in self.thumb_self.scanner_frame.winfo_children(): w.destroy()
        for i, page in enumerate(self.pages):
            ...
            # Card for thumbnail
            card = ctk.CTkFrame(self.thumb_self.scanner_frame, fg_color=COLORS["surface" if not is_selected else "primary"],
